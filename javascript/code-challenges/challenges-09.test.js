...
const maxInArray = (arr) => {
  let max = arr.reduce(function(accumulator, value, idx) {
    if (idx === 1) {
      accumulator = value;
      return accumulator
...
const checkValues = (obj, value) => {
  let inObjArr = Object.values(obj);
  let found = 0
  inObjArr.forEach(val => {
    if (val === value) {
      found = 1;
    } else {
      found = 0;
...
const updateNumbers = (obj) => {
  ...
  let joinedArr = keysArr.map((key, idx) => {
    let string = key + ': ' + valuesArr[idx]
    return string;
  })
  return joinedArr;
};
